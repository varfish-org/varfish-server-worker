...
impl StructuralVariant {
    ...
    fn build_call_info(
        ...
        for (idx, (name, sample)) in header
            .sample_names()
            .iter()
            .zip(record.samples().values())
            .enumerate()
        {
            ...
            for (key, value) in record.samples().keys().as_ref().iter().zip(sample.values()) {
                let value = value.as_ref().unwrap_or_else(|| {
                    panic!("empty FORMAT value for sample #{} (1-based)", idx + 1)
                });
                use noodles::vcf::variant::record::samples::keys::key;
                match (key.as_str(), value) {
                    (
                        key::GENOTYPE,
                        vcf::variant::record_buf::samples::sample::Value::String(gt),
                    ) => {
                        call_info.genotype = Some(gt.to_string());
                    }
                    (
